...
int main(int argc, char** argv) {
  ...
  char *outfile;
  outfile=argv[2];
  float rescalingFactor=atof(argv[3]);
  if(argc!=4) {
    printf("Usage: ./cutoutDm <infile> <outfile> <rescaling factor>\n");
    return 0;
  }
  printf(">> Got %g for rescaling factor \n",rescalingFactor);
  ...
  /* correct the masses */
  printf("Changing DM mass from %g to %g (+%g)\n",header.mass[1],header.mass[1]+rescalingFactor,rescalingFactor);
  header.mass[1]=P[0].Mass*rescalingFactor; /* add gas mass to dm mass in order to comply with rockstar */
  header.mass[0]=0;
  header.mass[4]=0;
  header.mass[5]=0;
